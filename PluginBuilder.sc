...
PluginBuilder {
	*generateCode { |synthName, params, builddir = false |
		...
		iterates.do( { |item, i|
			...
			~params.do({ |item i|
				var param = item[0].asString, min = item[1], max = item[2], default = item[3];
		...
		singles.do({|item, i|
			...
			final = "///" ++ position;
			final = final ++ code.replace("^^synth_name^^", synthName.toUpper)
			.replace("^^synth_name^", this.firstToUpper(synthName))
			.replace("^synth_name^", synthName)
		...
		batch = batch.replace("^synth_name^", synthName);
		in.close();
